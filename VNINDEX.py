...
def get_data_from_cophieu68_openwebsite(tickers):
    ...
    # file_url = f'https://www.cophieu68.vn/download/company.php?t=history&id={ticker}&pk=MTcxODg3MzY3OCNjcDY4IzIwMjQwNjIw'
    for ticker in tickers:

        webbrowser.open(
            f'https://www.cophieu68.vn/download/company.php?t=history&id={ticker}&pk=MTcxODg3MzY3OCNjcDY4IzIwMjQwNjIw')


def getliststocks(typestock="^VNINDEX"):
    benchmark = ["^VNINDEX", "^HASTC", "^UPCOM",
                 "VNINDEX", "UPINDEX", "HNXINDEX"]
    futures = ["VN30F1M", "VN30F2M", "VN30F1Q", "VN30F2Q"]

    nganhang = ['ACB', 'CTG', 'VPB', 'VCB', 'NVB', 'LPB',
                'VIB', 'BID', 'HDB', 'EIB', 'MBB', 'SHB', 'STB']

    thuysan = ['VHC', 'ANV']
    daukhi = ['PVS', 'PVD', 'PVB', 'PLX', 'BSR', 'POW', 'TDG', 'GAS', 'PLC']
    batdongsan = ['HAR', 'HLD', 'DXG', 'NVL', 'KDH', 'CEO',
                  'VIC', 'NDN', 'PDR', 'VPI', 'VRE', 'ASM', 'EVG', 'NBB']
    chungkhoan = ['HCM', 'SSI', 'VND', 'TVB', 'TVS', 'BVS',
                  'MBS', 'FTS', 'HCM', 'VIX', 'ART', 'SHS', 'VCI']
    baohiem = ['BVH', 'BMI']
...
# (GVR TDM MSH ACV BWE NTC VEA CTR ANV)
    ...
    if typestock == "HNXINDEX":
        symbols = symbolsHNX + [typestock]
    if typestock == "UPINDEX":
...
def analysis_trading(tickers, start, end, update=False, nbdays=15, source="cp68", trade='Long'):
    ...
    for ticker in tickers:
        try:
            res = hung_canslim(ticker, start, end, realtime=update,
                               source=source, ndays=nbdays, typetrade=trade)
            if len(res) > 1:
...
def passive_strategy(start_date, end_date, market="VNINDEX", symbols=None, realtime=False, source='cp68'):
    ...
    df_data = get_data(symbols, dates, benchmark=market,
                       realtime=realtime, source=source)  # get data for each symbol
    # Fill missing values
    fill_missing_values(df_data)

    df_volume = get_data(symbols, dates, benchmark=market, colname='Volume',
                         realtime=realtime, source=source)  # get data for each symbol
    df_high = get_data(symbols, dates, benchmark=market,
                       colname='High', realtime=realtime, source=source)
    df_low = get_data(symbols, dates, benchmark=market,
                      colname='Low', realtime=realtime, source=source)
...
#    beta = covariance / variance
    ...
    df_result['PCT_C'] = 100*(df_data[symbols].iloc[-1, :].values -
                              df_data[symbols].iloc[0, :].values)/df_data[symbols].iloc[0, :].values
...
#    df_result['MinL'] = min_low
    ...
    df_result['PCT_Sum4D'] = df_result['PCT_3D'] + \
        df_result['PCT_2D'] + df_result['PCT_1D'] + df_result['PCT_0D']

    df_result['Vol_ratio'] = df_volume[symbols].iloc[-1,
                                                     :].values/volumeM30[symbols].iloc[-1, :].values
...
def analysis_stocks(start_date, end_date, realtime=True, source='cp68'):
    ...
    hsx_res, hsx_data, hsx_market = passive_strategy(
        start_date=start_date, end_date=end_date, market="VNINDEX", realtime=realtime, source=source)
    hnx_res, hnx_data, hnx_market = passive_strategy(
        start_date=start_date, end_date=end_date, market="HNXINDEX", realtime=realtime, source=source)
    upcom_res, upcom_data, upcom_market = passive_strategy(
        start_date=start_date, end_date=end_date, market="UPINDEX", realtime=realtime, source=source)
...
if __name__ == "__main__":
    ...
    while trading:
        ...
        if (t >= t1 and t <= t2) or (t >= t3 and t <= t4) and realtime:
            os.system('cls')
            print('TRADING SYSTEM SIGNAL...............',
                  time.asctime(time.localtime(time.time())))
            res = analysis_trading(tickers=None, start=start_date, end=end_date,
                                   update=realtime, nbdays=nlastdays, source=datasource, trade=trademethod)
            ...
            waittime = t1 - t
            print("WAIT FOR {} MINUTES FROM NOW {} OR COME BACK LATER...".format(
                waittime, trade_time))
            res = analysis_trading(tickers=None, start=start_date, end=end_date,
                                   update=False, nbdays=nlastdays, source=datasource, trade=trademethod)
            print(res.to_string())
            if not np.is_busday(today) or today in vn_holidays:
                trading = False
                break
            ...
            print('TRADING SYSTEM SIGNAL DURING BREAK...............',
                  time.asctime(time.localtime(time.time())))
            res = analysis_trading(tickers=None, start=start_date, end=end_date,
                                   update=realtime, nbdays=nlastdays, source=datasource, trade=trademethod)
            print(res.to_string())
            ...
            print('LAST-MINUTE TRADING SYSTEM SIGNAL...............',
                  time.asctime(time.localtime(time.time())))
            res = analysis_trading(tickers=None, start=start_date, end=end_date,
                                   update=realtime, nbdays=nlastdays, source=datasource, trade=trademethod)
            print(res.to_string())
    ...
    stock_all, market_all = analysis_stocks(
        start_date=start_date, end_date=end_date, realtime=False, source='amibroker')
