# -*- coding: utf-8 -*-
"""
Created on Sun Apr 12 11:35:20 2020
...
stocks = ["FPT", "MWG", "VNM", "PNJ"]  # the 4 stocks we want to consider

start ="2014-12-13"
end = "2020-4-10"

for stock in stocks:  # begin iteration

    stock = stock.split('.csv')[0]  # split away the ticker from the file-name
    print(stock)
    dataset = f'cp68/excel_{stock}.csv'  # get the full path to the file.
    
    df = pd.read_csv(dataset, index_col ="<DTYYYYMMDD>", parse_dates = True, 
    ...
    df = df[[f"{stock}_close", f"{stock}_volume"]]  # ignore the other columns besides price and volume
...
tensorboard = TensorBoard(log_dir="logs\\{}".format(STOCK_TO_PREDICT))
