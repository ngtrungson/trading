...
#    
    ...
    ax0.plot(df_ohlc['Date'], df_ohlc['RSI'].values, mfiCol, linewidth=1)
    ax0.axhline(60, color=negCol)
    ax0.axhline(30, color=posCol)
    ax0.fill_between(df_ohlc['Date'], df_ohlc['RSI'].values, 60, where=(df_ohlc['RSI']>=60), facecolor=negCol, edgecolor=negCol, alpha=0.5)
    ax0.fill_between(df_ohlc['Date'], df_ohlc['RSI'].values, 30, where=(df_ohlc['RSI']<=30), facecolor=posCol, edgecolor=posCol, alpha=0.5)
    ax0.set_yticks([30,60])
...
def plot_canslim_trading(ticker, df):
...
#Converting dates column to float values
    ...
    ax1.plot(df_ohlc['Date'],df_ohlc['SMA15'].values, linewidth=1, label = 'SMA15')
    ax1.plot(df_ohlc['Date'],df_ohlc['SMA30'].values, linewidth=1,label = 'SMA30')
